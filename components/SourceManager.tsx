      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        if (pageText.trim().length < 50) {
           ...
           if (context) {
             ...
             // Set preview for UI
             setOcrPreviewUrl(canvas.toDataURL());
          ...
          setOcrPreviewUrl(null); // Clear preview if normal text extraction
        }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            >
              {isProcessing && (
                <div className="absolute inset-0 bg-white bg-opacity-95 z-10 flex flex-col items-center justify-center p-6 text-center">
                   
                   {ocrPreviewUrl ? (
                     <div className="mb-3 flex flex-col items-center animate-in fade-in slide-in-from-bottom-2">
                       <span className="text-[10px] text-gray-500 uppercase font-bold mb-1 tracking-wider">Escanejant document</span>
                       <div className="p-1 bg-white border border-gray-200 shadow-lg rounded max-w-[150px]">
                   ...
                   <p className="text-sm font-semibold text-indigo-700 animate-pulse">{statusMsg}</p>
            ...
            {uploadedFiles.length > 0 && (
              <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                 <h4 className="text-xs font-semibold text-gray-500 uppercase mb-2">Fitxers carregats ({uploadedFiles.length})</h4>
                 <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar">
                   {uploadedFiles.map((f, idx) => (
                     <div key={idx} className="flex items-center justify-between text-sm bg-white p-2 rounded border border-gray-200 hover:border-indigo-200 transition-colors">
                       <div className="flex items-center truncate flex-1 mr-2">
                         <FileIcon size={14} className="text-gray-400 mr-2 flex-shrink-0" />
                         <span className="truncate text-gray-700" title={f.name}>{f.name}</span>
                       </div>
                       {f.type === 'application/pdf' && (
                         <button 
                           onClick={() => setFileToView(f)}
                           className="flex items-center space-x-1 text-xs text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-2 py-1 rounded transition-colors flex-shrink-0"
